...
#
import sys
sys.path.append('/home/disk/eos4/muting/function/')
...
def find_nfile_skip(CASENAME, CASENAME_SHORT, iyr_min, iyr_max):
    ...
    file_dir = '/glade/scratch/muting/work_output_pressure_coord_full/'+CASENAME+'/'
...
def load_2D_data_as_1variable(CASENAME,vname1, vname2, iyr_min, iyr_max, latmax, org2post_const, nfile_skip, KW_proj):
    ...
    for iyr in range(iyr_min,iyr_max+1):
        ...
        for imon in range(0,nmon):
            mon_str = "%02d" %(imon+1)
            # Load 3-hourly data
            file_dir = '/glade/scratch/muting/archive/'+CASENAME+'/atm/hist/'
...
#####################################
def calculate_PCWS(Tproj, Zproj, Wproj, EOF1_Q, EOF2_Q, plev, remove_lowpass=1):
    # This code is to calculate PC1 and PC2 for adiabatic cooling and heating Sp*Omega
    # Tproj is (time, plev, lon)
    # remove_lowpass=1 only for CAM6 model
    # remove_lowpass=0 for MPAS
    # note that this code is slightly different in CAM6 and ERA5 (see below "caution"
    ...
    if remove_lowpass == 1:
    	Spw = MJO.remove_lowpass_from_3hr_data(Spw)
    Spw = np.reshape(Spw,(nt*nlon,nlev-2))
    
    #################################
    # Caution: below is different for ERA5 and CAM6
    ####################################
    #PC1_WS = np.matmul(Spw, EOF1_Q[1:-1]) # for CAM6
    #PC2_WS = np.matmul(Spw, EOF2_Q[1:-1]) # for CAM6
    PC1_WS = np.matmul(Spw, EOF1_Q) # for ERA5
    PC2_WS = np.matmul(Spw, EOF2_Q) # for ERA5
    ####################################
