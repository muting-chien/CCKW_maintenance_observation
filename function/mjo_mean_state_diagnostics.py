...
# This is the correct version of it. (2022.10.10)
def remove_anncycle_new(x,dt): #x is one dimension, dt represents how many days in 1 time interval (3hourly-->dt=1/8, daily-->dt=1)
    mmax = 6 # remove mean and first 3 harmonics
    n = np.size(x)
    t = np.arange(0,n,1)
    xm = np.nanmean(x)
    x = np.matrix(x).T
    if xm!=np.nanmean(x):
        print('Caution: there might be nan in the data and you should not use this function!')
    xa = x-xm

    A = np.zeros([mmax,n])
    for m in range(0,mmax):
        if np.mod(m,2) == 0:
            A[m,:] = np.ones([1,n])*np.sin((m+2)/2*2*np.pi/365*dt*t)
        else:
            A[m,:] = np.ones([1,n])*np.cos((m+1)/2*2*np.pi/365*dt*t)
    C = 2*np.matmul(A,xa).T/n
    tmp = np.matmul(C,A).T 
    # Make sure tmp has the mean of zero
    tmp = tmp-np.nanmean(tmp)
    xcycle = tmp + xm
    xp = x-xcycle
    return xp, xcycle

def remove_anncycle_2d(x,time,lon,dt):#dt represents how many days in 1 time interval (3hourly-->dt=1/8, daily-->dt=1)
    ...
    for ilon in range(0,np.size(lon,0)):
            x_a, x_c=  remove_anncycle_new( np.squeeze(x[:,ilon]),dt )       
...
def remove_anncycle_3d(x,time,lat,lon,dt): #dt represents how many days in 1 time interval (3hourly-->dt=1/8, daily-->dt=1)
    # caution: before using remove_anncycle function, make sure there is no nan in the data, otherwise it won't work
    ...
    for ilat in range(0,np.size(lat,0)):
        for ilon in range(0,np.size(lon,0)):
            x_a, x_c=  remove_anncycle_new( np.squeeze(x[:,ilat,ilon]),dt )         
            x_ano[:,ilat,ilon] = np.squeeze(x_a)
            x_cyc[:,ilat,ilon] = np.squeeze(x_c)         
    print("finish removing anncycle")
    return x_ano, x_cyc

def remove_anncycle_4d(x,time,lev,lat,lon, dt):
    # caution: before using remove_anncycle function, make sure there is no nan in the data, otherwise it won't work
    ...
    for ilev in range(0,np.size(lev,0)):
        for ilat in range(0,np.size(lat,0)):
            for ilon in range(0,np.size(lon,0)):
                x_a, x_c=  remove_anncycle_new( np.squeeze(x[:,ilev,ilat,ilon]),dt )       
...
Applying filtering
'''
def remove_lowpass_from_3hr_data(x, fs=8, cutoff=1/30):
    time_dim = 0
    # fs = 8-->8 samples per day (3-hourly)
    # cutoff = 1/30 (1/day)-->cut off signals with period longer than 30 day
